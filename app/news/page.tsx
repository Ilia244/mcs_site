...
type P={id:string;title:string;content:string;created_at:string;published_at:string;type:string;status:string;tags:Tag[]}
...
export default function NewsPage(){
  ...
  useEffect(()=>{(async()=>{const[{data:posts},{data:allTags}]=await Promise.all([
    supabase.from("posts").select("id,title,content,created_at,published_at,type,status,post_tags(tags(name,color,icon,slug))").eq("status","published"),
  ...
  return <div className="portal-bg min-h-screen text-white"><main className="relative z-10 max-w-5xl mx-auto px-5 py-14"><Link href="/" className="text-cyan-300 text-sm">← ホームへ戻る</Link><div className="mt-6 mb-8"><p className="section-kicker">INFORMATION</p><h1 className="text-4xl md:text-5xl font-bold">お知らせ</h1><p className="text-gray-400 mt-3">運営情報・イベント・MCS・配信・YouTubeなどの情報。</p></div>
    ...
    <p className="text-sm text-gray-500 mb-4">{filtered.length}件</p><div className="news-list space-y-4">{loading?<div className="portal-panel text-gray-400">お知らせを読み込み中...</div>:filtered.length?filtered.map(p=><Link href={`/news/${p.id}`} key={p.id} className="portal-panel news-item block-hover"><div className="flex gap-2 flex-wrap">{p.tags?.map(t=><span key={`${p.id}-${t.name}`} className="tag-pill" style={{borderColor:t.color,color:t.color}}>{t.icon} {t.name}</span>)}</div><p className="text-xs text-gray-500 mt-3">{new Date(p.published_at||p.created_at).toLocaleDateString("ja-JP")}</p><h2 className="text-2xl font-bold mt-2">{p.title}</h2><p className="text-gray-300 mt-4 whitespace-pre-wrap leading-relaxed line-clamp-4">{p.content}</p></Link>):<div className="portal-panel text-gray-400">条件に一致するお知らせはありません。</div>}</div></main></div>
}
